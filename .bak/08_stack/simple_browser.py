...
'''
    ...
    Author: guangshu.xu
    解答：使用两个栈，X 和 Y，把首次浏览的页面依次压入栈 X，当点击后退按钮时，再依次从栈 X 中出栈，
    并将出栈的数据依次放入栈 Y。当点击前进按钮时，我们依次从栈 Y 中取出数据，放入栈 X 中。
...
class Browser:
    ...
    def can_forward(self):
        if self.forward_stack.if_empty():
            return False

    def can_back(self):
        if self.back_stack.if_empty():
            return False
